...
const ProcessFeed = ({ data = [] }) => {
    ...
    const formatVideo = (media) => {
        ...
        if (video.url) {
            ...
            content = `<br><video src="https://proxy.andiedie.cn/${encodeURIComponent(video.url)}?proxy=true" ${gifAutoPlayAttr} controls="controls" poster="https://proxy.andiedie.cn/${encodeURIComponent(getOrigionImg(media.media_url_https))}?proxy=true" style="width: 100%"></video>`;
        }
        ...
        item.extended_entities &&
            item.extended_entities.media.forEach((item) => {
                ...
                switch (item.type) {
                    ...
                    case 'photo':
                    default:
                        content = `<br><img referrerpolicy="no-referrer" src="https://proxy.andiedie.cn/${encodeURIComponent(getOrigionImg(item.media_url_https))}?proxy=true">`;
                        break;
                }
