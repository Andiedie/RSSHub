const axios = require('../../utils/axios').proxy;
        ...
        item: list.map((item, index) => {
            ...
            for (let i = 0; i < media[index].length; i++) {
                if (media[index][i].image) {
                    content += `<img referrerpolicy="no-referrer" src="https://proxy.andiedie.cn/${encodeURIComponent(media[index][i].image)}?proxy=true"><br>`;
                    image++;
                }

                if (media[index][i].video) {
                    content += `<video width="100%" controls="controls"> <source src="https://proxy.andiedie.cn/${encodeURIComponent(media[index][i].video)}?proxy=true" type="video/mp4"> Your RSS reader does not support video playback. </video>`;
                    video++;
                }
            }
